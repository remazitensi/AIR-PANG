...
const MonthlyAqi = () => {
    ...
    useEffect(() => {
        ...
        const fetchSubLocations = async () => {
            if (selectedLocation) {
                try {
                    const response = await axios.get(`http://localhost:8080/sublocations?location=${selectedLocation}`);
    ...
    useEffect(() => {
        const fetchData = async () => {
            if (selectedLocation && selectedSubLocation) {
                try {
                    setLoading(true);
                    const response = await axios.get(`http://localhost:8080/location/detail`, {
                        params: {
    ...
    // 차트 데이터 준비
    const chartData = data && data.monthly_aqi ? {
        labels: data.monthly_aqi.slice(-12).map(aqi => aqi.month),
        datasets: [{
            label: 'Monthly AQI',
            data: data.monthly_aqi.slice(-12).map(aqi => aqi.aqi),
